class Business::SubscribersController < ApplicationController
  layout 'business'
  ...
  # GET /subscribers.xml
  def index
    @origins = Origin.find(:all, :conditions => {:business_id => current_user.business.id})
    @subscribers = User.find(:all, :conditions => {:origin_id  => @origins, :gets_daily_deal_email => true})
